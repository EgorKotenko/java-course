...
public class List {
    ...
    */
    public void removeElementFromList(int number) throws removeNullElementExeption{
        if ((existInList(number) == false) || (isEmpty()))
            throw new removeNullElementExeption(" removing non-existent element");
        if (isEmpty()) {
    ...
    */
    ...
    /**
    * Проверяет, присутствует ли в списке элемент, равный данному числу.
    * @author Егор
    * @param number Число, вхождение в список которого мы проверяем.
    */
    public boolean existInList(int number) {
        if (isEmpty())
            return false;
        else{
            Node index = first;
